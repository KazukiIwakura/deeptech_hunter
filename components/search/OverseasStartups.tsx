...
const SkeletonLoader: React.FC = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[...Array(2)].map((_, i) => (
            <div key={i} className={cn('bg-white/70 backdrop-blur-md border border-neutral-200/80 rounded-4xl shadow-sm transition-all duration-300', "p-6 flex flex-col h-full animate-pulse")}>
                <div className="flex-grow">
                    <div className="h-4 bg-neutral-200 rounded w-1/3 mb-2"></div>
                    <div className="h-6 bg-neutral-200 rounded w-3/4 mb-4"></div>
                    
                    <div className="h-8 bg-neutral-200 rounded-full w-2/5 mb-5"></div>
                    ...
                    <div className="mb-5 p-4 bg-neutral-50/70 rounded-xl border border-neutral-200/60 space-y-3">
...
export const OverseasStartups: React.FC<OverseasStartupsProps> = ({ startups, isLoading, error }) => {
    ...
    return (
        <div className={cn('border-l-4 p-6 rounded-2xl', 'bg-neutral-100/70 border-neutral-300 backdrop-blur-sm', "text-center p-8 border-none")}>
            <p className="font-bold text-main-light">関連する海外スタートアップは見つかりませんでした。</p>
            <p className={cn('text-sm text-main-lighter leading-normal', "mt-1")}>まだ市場が形成されていない、非常に新しい分野の可能性があります。</p>
        </div>
    );
};
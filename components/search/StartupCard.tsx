...
const InfoItem: React.FC<{
  ...
  return (
    <div className={cn("flex items-start justify-between", 'text-sm text-main-light leading-normal')}>
        <div className="flex items-center text-main-light flex-shrink-0 pr-4">
            {icon}
            <span className="font-normal">{label}</span>
        </div>
        <span className="text-right text-main font-normal break-words">{value}</span>
    </div>
  );
};
    ...
    <Card className="p-6 flex flex-col h-full hover:border-primary/80 hover:shadow-lg hover:-translate-y-1 animate-fade-in">
      <div className="flex-grow">
        ...
        <div className="mb-8 p-4 bg-neutral-50/70 rounded-2xl border border-neutral-200/60 space-y-4">
            <h4 className={cn('text-base leading-snug font-bold text-main', "text-main-light")}>VC's Eye View</h4>
            <InfoItem icon={<BriefcaseIcon className="w-4 h-4 mr-2 text-neutral-500"/>} label="Business Model" value={startup.business_model} />
            <InfoItem icon={<VialIcon className="w-4 h-4 mr-2 text-neutral-500"/>} label="Core Technology" value={startup.technology_summary} />
            <InfoItem icon={<TrendingUpIcon className="w-4 h-4 mr-2 text-neutral-500"/>} label="Latest Milestone" value={startup.latest_milestone} />
        </div>

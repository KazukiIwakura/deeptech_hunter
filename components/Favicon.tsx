...
export const Favicon: React.FC<FaviconProps> = ({ domain, alt, className }) => {
    ...
    if (hasError || !src) {
        return (
            <div className={cn("flex items-center justify-center bg-slate-100 border border-slate-200", className)} aria-label={altText}>
                <GlobeIcon className="w-1/2 h-1/2 text-slate-400" />
            </div>
        );
    }
    ...
    return (
        <img 
            ...
            onError={handleError} 
            alt={altText} 
            className={cn("bg-white border border-slate-200 object-cover", className)}
            aria-hidden="true"
        />
    );
};